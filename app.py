...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    .metric-card h3 { color: #aaa; font-size: 0.9rem; text-transform: uppercase; }
    .metric-card h2 { color: white; font-size: 2.2rem; margin: 0.5rem 0; }
    .cost-card {
        border-radius: 24px;
        padding: 2rem;
        text-align: center;
    ...
    .cost-low { background: linear-gradient(135deg, #11998e, #38ef7d); }
    .cost-medium { background: linear-gradient(135deg, #f2994a, #f2c94c); }
    .cost-high { background: linear-gradient(135deg, #eb3349, #f45c43); }
    @keyframes fadeInUp {
    ...
    .stTabs [aria-selected="true"] { background: linear-gradient(135deg, #667eea, #764ba2); }
...
if 'example_sql' not in st.session_state:
    st.session_state.example_sql = ""   # stores the SQL from example buttons
...
def calculate_cost(query_text, mb_estimate):
    ...
    base = gb * 0.50
    complexity = 1.0
    if 'JOIN' in query_text.upper(): complexity *= 1.5
    if 'GROUP BY' in query_text.upper(): complexity *= 1.3
    if 'ORDER BY' in query_text.upper(): complexity *= 1.1
    if 'DISTINCT' in query_text.upper(): complexity *= 1.2
    if 'WHERE' in query_text.upper(): complexity *= 0.7
    if 'LIMIT' in query_text.upper(): complexity *= 0.5
...
def get_cost_color(cost):
    if cost < 0.01: return "cost-low"
    elif cost < 0.10: return "cost-medium"
    else: return "cost-high"
...
with tab1:
    ...
    with col_left:
        # Use the example_sql as default value
        query = st.text_area(
            "✍️ **SQL Query**",
            value=st.session_state.example_sql,
            height=150,
    ...
    with col_right:
        ...
        for name, sql in examples.items():
            if st.button(name, key=f"ex_{name}"):
                # Store the SQL in session state and rerun to update the text area
                st.session_state.example_sql = sql
                st.rerun()
...
LIMIT 1000;
        """, language='sql')
    st.markdown("### 🧪 Try this optimized query")
